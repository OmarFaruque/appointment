...
*/
	...
	/* To Embed Dashboard on Frontend */
    add_shortcode('admin_sc','apt_admin_dashboard');
    
	add_shortcode('staff_sc','apt_staff_dashboard');
	add_shortcode('client_sc','apt_client_dashboard');

	function apt_admin_dashboard(){
	  global $wp_styles;
		ob_start();
		if(is_user_logged_in()){
			...
			?>
			<script>
			function apt_admin_iframemodify(){ 
				jQuery('#apt_admin_dashboard_style').contents().find('#wpadminbar').remove();
				jQuery('#apt_admin_dashboard_style').contents().find('#adminmenuback').remove();
				jQuery('#apt_admin_dashboard_style').contents().find('#adminmenuwrap').remove();
				jQuery('#apt_admin_dashboard_style').contents().find('#wpfooter').remove();
				jQuery('#apt_admin_dashboard_style').contents().find('head').append('<style>html.wp-toolbar{padding-top: unset !important;}#wpcontent{margin-left: unset !important;}</style>'); 
			}
			
			</script>
			<iframe onload='apt_admin_iframemodify()' scrolling="auto" class="iframe-full-height" id="apt_admin_dashboard_style" style="" src="https://appts.ufound.us/wp-admin/admin.php?page=dashboard_submenu"></iframe>
	...
	global $wp_styles;
		...
		if(is_user_logged_in()){
			...
			?>
			<script>
			function apt_staff_iframemodify(){ 
				jQuery('#apt_staff_dashboard_style').contents().find('#wpadminbar').remove();
				jQuery('#apt_staff_dashboard_style').contents().find('#adminmenuback').remove();
				jQuery('#apt_staff_dashboard_style').contents().find('#adminmenuwrap').remove();
				jQuery('#apt_staff_dashboard_style').contents().find('#wpfooter').remove();
				jQuery('#apt_staff_dashboard_style').contents().find('head').append('<style>html.wp-toolbar{padding-top: unset !important;}#wpcontent{margin-left: unset !important;}</style>');
			}
			     //obj.style.height = obj.contentWindow.document.body.scrollHeight + 'px';
			</script>
			<iframe  onload='apt_staff_iframemodify()' id="apt_staff_dashboard_style" style="max-width:100%;" height="600px" width="940px" src="https://appts.ufound.us/wp-admin/admin.php?page=provider_submenu"></iframe>
			<?php
	...
	global $wp_styles;
		...
		if(is_user_logged_in()){
			...
			?>
			<script>
			function apt_client_iframemodify(){ 
				jQuery('#apt_client_dashboard_style').contents().find('#wpadminbar').remove();
				jQuery('#apt_client_dashboard_style').contents().find('#adminmenuback').remove();
				jQuery('#apt_client_dashboard_style').contents().find('#adminmenuwrap').remove();
				jQuery('#apt_client_dashboard_style').contents().find('#wpfooter').remove();
				jQuery('#apt_client_dashboard_style').contents().find('head').append('<style>html.wp-toolbar{padding-top: unset !important;}#wpcontent{margin-left: unset !important;}</style>');
			}
			</script>		
			<iframe  onload='apt_client_iframemodify()' id="apt_client_dashboard_style"  style="max-width:100%;" height="600px" width="940px" src="https://appts.ufound.us/wp-admin/admin.php?page=dashboard_submenu"></iframe>
			<?php 
   ...
   global $wpdb;	  	
	...
	function appointment_admin_scripts($hook) {
		...
		if ( 'admin_page_appointments_submenu' == $hook || 'toplevel_page_appointment_menu' == $hook ) {
			...
			if(current_user_can('manage_options') || current_user_can('apt_provider') || current_user_can('apt_staff')) {
